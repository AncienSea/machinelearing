...
def build_report(args: argparse.Namespace) -> None:
    ...
    story.append(
        p(
            "从误差来源看，测试集上的极端波动并不是某一种模型独有的问题，而是由负荷序列自身的突变、节假日扰动和外部气象因素共同造成的。"
            "在只使用历史功率和日历特征的设定下，模型能较好学习周期性和局部连续性，却很难提前知道某一天是否会出现异常峰值。"
            "因此，图中的局部误差更适合被理解为“信息缺失”带来的系统性偏差，而不是简单的训练不足。"
            "这也解释了为什么对短期任务而言，CNN-Transformer 的局部建模更占优，而长周期任务更依赖整体序列依赖；若未来补充天气、节假日、家庭活动或价格信号，误差分布有望进一步收敛。",
            styles,
        )
    )
    story.append(
        p(
            "从工程实现看，本次实验的重点并不只是给出一个最优数值，而是构建一条可以被复用的闭环流程：原始数据处理、特征工程、模型训练、指标汇总、图表生成和 PDF 报告输出都由同一套脚本串联。"
            "这样做的好处在于，任何一张截图都能追溯到对应的预测文件和指标文件，任何一次改动也都可以通过重新运行脚本自动同步到图表和正文。"
            "对课程作业来说，这种“结果可解释、过程可重建、结论可验证”的组织方式，本身就是实验质量的一部分。",
            styles,
        )
    )
    story.append(
        p(
            "从评价指标看，MAE 与 MSE 提供了互补视角：MAE 更接近平均误差的直观含义，便于解释“平均偏离了多少”；"
            "MSE 对少量大误差更加敏感，因此能把极端峰值拉出来单独观察。将二者同时报告，可以避免只看单一指标造成的偏差，"
            "也能更清楚地比较不同模型在“整体稳定性”和“极端误差控制”上的差别。这也是为什么本报告在正文中既保留均值，也保留标准差与单 seed 图像截图。",
            styles,
        )
    )
